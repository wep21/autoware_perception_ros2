  ...
  tf_listener_(tf_buffer_)
{
  ...
  const auto width = node_->declare_parameter("width").get<int>();
  const auto height = node_->declare_parameter("height").get<int>();
  const auto caffemodel_file = node_->declare_parameter("caffemodel_file").get<std::string>();
  const auto prototxt_file = node_->declare_parameter("prototxt_file").get<std::string>();
  const auto engine_file = node_->declare_parameter("engine_file").get<std::string>();
  const auto use_intensity_feature = node_->declare_parameter("use_intensity_feature").get<bool>();
  const auto use_constant_feature = node_->declare_parameter("use_constant_feature").get<bool>();
  target_frame_ = node_->declare_parameter("target_frame").get<std::string>();
  z_offset_ = node_->declare_parameter("z_offset").get<float>();
  // load weight file
  ...
  float z_offset)
{
  ...
  sensor_msgs::msg::PointCloud2 pointcloud_with_z_offset;
